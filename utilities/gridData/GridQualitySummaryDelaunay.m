function [AreaRatioQuality, ShapeQuality, AngleQuality, Skewness] = GridQualitySummaryDelaunay(triMesh, invalidCellIndex, xCoord, yCoord)
if nargin == 2
...
%% 计算网格角度质量, 只考虑有效单元的网格质量
nTotalCells = size(triMesh,1);
nInvalid    = length(invalidCellIndex);
nValid      = nTotalCells - nInvalid;
cellNodeTopology = zeros(nValid,3); 
count = 1;
for i=1:size(triMesh,1)
    if sum(i==invalidCellIndex)==0
        cellNodeTopology(count,:) = triMesh.ConnectivityList(i,:);
        count = count + 1;
    end
end
[AngleQuality, Skewness] = GridAngleQualitySummary(cellNodeTopology, xCoord, yCoord);
...
%%
AreaRatioQuality = [minAreaRatio; maxAreaRatio; averAreaRatio];
ShapeQuality = [minQuality; maxQuality; averQuality];
end
